...
const Calousel = ({ Display }) => {
  ...
  const { content } = CustomHooks(
    `https://api.themoviedb.org/3/discover/${Display}?api_key=97cd0ab3fd3e22a15f551b94745e328a&language=${lang}`
  );
      ...
      {content.length > 0 ? (
        ...
        >
          {content.map((movie) => {
            return (
              <div
                className="item px-2 position-relative overflow-hidden"
                key={movie.id}
              >
                <span
                  href="#"
